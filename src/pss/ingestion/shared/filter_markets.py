...
RELEVANT_TAGS = {
    ...
    "google", "automation", "amodei", "predictions",

    # Semiconductors
    "nvda", "tech",

    # Crypto
    "bitcoin", "ethereum", "crypto", "solana", "dogecoin", "token", "airdrops",

    # Equities & Markets
    "tsla", "tesla", "spacex", "stocks", "s&p", "spx", "equities", "equity",
    "indicies", "ipo", "ipos", "liquidate", "derivative", "futures",
    "pre-market", "markets", "sales", "price", "prices", "jpm",

    # Macro / Fed
    "fed", "fomc", "inflation", "rates", "cpi", "federal", "reserve",
    "interest", "policy", "powell", "chair", "bank", "macro", "economic",
    "economy", "finance", "business", "taxes", "shutdown", "congress",
    "presidency", "gov", "recurring",

    # Commodities
    "gold", "silver", "oil", "crude", "gas", "commodities",
    "comex", "nymex", "nyc",

    # Trade & Geopolitics
    "tariffs", "trade", "sanctions", "embargo", "china", "taiwan",
    "nuclear", "enrich", "military", "regime", "risk",

    # Strait of Hormuz / Energy Supply
    "hormuz", "ships", "transit", "toll", "space",
...
def _clean_tag_words(tags: list[str]) -> set[str]:
    clean_words = set()
    for tag in tags:
...
def _keep_by_tags(tags: list[str], threshold: float = 0.5) -> bool:
    if not tags:
        return True

    cleaned_market_words = _clean_tag_words(tags)

    if not cleaned_market_words:
        return True

    return len(cleaned_market_words.intersection(RELEVANT_TAGS))/ len(cleaned_market_words) >= threshold
